<template>
  <div>
    ...
    <main class="col-sm-12 col-md-9 col-lg-10 px-md-4 offset-md-2 p-5 pt-3">
      <h2 class="mb-3">Envoyer une nouvelle diffusion</h2>
      ...
      <label class="form-control-label mb-1 ml-2" for="users">Abonné(s)</label>
      <sui-dropdown
          id="users"
          fluid
          :options="users"
      ...
      />
      <sui-button v-on:click="sendMail" :loading="submitting"  content="Envoyer la diffusion" color="black"
...
export default {
  ...
  mounted() {
    ...
    this.loading = true;
    axios.get(`${config.server}/news-letter`, {headers: {...config.headers}}).then((data) => {
      let subscribers = data.data;
      this.users = subscribers.map(({ email}) => {
        return {
          text: `${email}`,
          value: `${email}`,
          key: `${email}`,
      ...
      this.selectedUsers=this.users.map((user)=>user.value);
  ...
  methods: {
    ...
    sendMail() {
      ...
      axios.post(`${config.server}/emails/send`,{users:selectedUsers,subject,body,isNewsletter:true},{headers: {...config.headers}}).then(()=>{
        context.submitting=false;
        Vue.$toast.open({message: 'Courrier envoyé avec succès!', type: 'success'})
        context.$router.push('/admin/newsletter')
      }).catch(()=>{
